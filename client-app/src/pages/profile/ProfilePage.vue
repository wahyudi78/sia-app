<script setup>
import { computed, ref, onMounted, onBeforeMount } from "vue";

import { allGuru, deleteGuru } from "../../http/guru";
import Navbar from "../../components/Navbar.vue";

const isEdit = ref(false);
const user = JSON.parse(localStorage.getItem("datauser"));
</script>

<template>
  <Navbar />
  <div class="conatiner m-3">
    <div class="row justify-content-center">
      <div class="col-md-2">
        <div class="card shadow rounded p-3 d-flex">
          <div class="container">
            <img
              src="data:image/png;base64,"
              alt=""
              width="100"
              srcset=""
            />
          </div>
        </div>
      </div>
      <div class="row">
        <div class="col-md">
          <div class="card shadow rounded p-5">
            <form>
              <div class="form-group">
                <label for="username">Username</label>
                <input :disabled="!isEdit" :value="user.name" type="text" class="form-control" id="username" />
              </div>
              <div class="form-group">
                <label for="alamat">alamat</label>
                <input :disabled="!isEdit" :value="user.nomor_induk" type="text" class="form-control" id="alamat" />
              </div>
              <div class="form-group">
                <label for="alamat">alamat</label>
                <input :disabled="!isEdit" :value="user.alamat" type="text" class="form-control" id="alamat" />
              </div>

              <button type="submit" class="btn btn-primary">Submit</button>
            </form>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>
